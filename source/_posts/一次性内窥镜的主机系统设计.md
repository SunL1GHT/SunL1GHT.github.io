...
- nearest（邻域像素重复技术）——放大像素大小但不增加细节，引起锯齿现象
  ...
  ![](https://cdn.jsdelivr.net/gh/SunL1GHT/ImagePicGo@main/img/20210326185216.jpg)

- bilinear（双线性图像插值）——相似三角形原理，均值插值，以周围四个点来得到中间点的值，解决锯齿，但会产生模糊。

  ![](https://cdn.jsdelivr.net/gh/SunL1GHT/ImagePicGo@main/img/20210326185317.jpg)

- bicubic（双三次图像插值），插值函数R. G. Keys, “Cubic convolution interpolation for digital image processing,” IEEE Trans. On Acoustics, Speech, and Signal Processing, ASSP-29(6): 1153-1160, 1981.
  $$
  \text{g(x)=}\sum\limits_{k}{{{c}_{k}}\cdot u(\frac{x-{{x}_{k}}}{h})}\
  $$

  $$
  u(s)=\left\{ \begin{matrix}
     {{A}_{1}}|s{{|}^{3}}+{{B}_{1}}|s{{|}^{2}}+{{C}_{1}}|s|+{{D}_{1}} & 0<|s|<1  \\
     {{A}_{2}}|s{{|}^{3}}+{{B}_{2}}|s{{|}^{2}}+{{C}_{2}}|s|+{{D}_{2}} & 1<|s|<2  \\
     0 & 2<|s|  \\
  \end{matrix} \right.
  $$
...
- 边缘导向差值法——假设自然图像可以用局部的高斯随机过程来描述，基于几何对偶性算出协方差系数使得LR→HR。图片给更光滑，速度快。
  $$
  \vec{\alpha}=\left(C^{T} C\right)^{-1}\left(C^{T} \vec{y}\right)
  $$
...
- Superresolution，ICCV2009_D. Glasner, S. Bagon, and M. Irani, “Super-resolution from a single image,” In Proc. of ICCV, 2009.  
