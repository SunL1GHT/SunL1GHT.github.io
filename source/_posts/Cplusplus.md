...
*一定要在指针应用解引用运算符*之前，将指针初始化为一个确定的的、适当的地址。*
...
变量是在编译时分配的有名称的内存，指针在运行阶段分配未命名的内存以存储值。而在运行阶段需要为哪一种数据类型分类内存，我们会使用`new`关键字。

`new`分配内存，`int *pn=new int;//new int 告诉程序，需要适合存储int的内存，pn指向的是内存而不是变量`

new的动态联编的特性，使得它更适用于大型数据（数组、字符串、结构体等）。
* 静态联编：在程序编译时分配内存空间。在创建数组时，不论程序是否使用都将占用一定内存。
* 动态联编：在程序运行时分配内存空间。在创建数组时，程序将在运行时确定数组的长度。

变量的值和指针指向的值都存储在栈（stack）区，而`new`从堆（heap）区或者自由存储区（free store）开辟内存。

`delete`释放内存，`delete pn;//释放pn指向的内存，但不会删除指针pn本身，因此可以将pn重新指向另一个新分配的内存块。`

> 一定要配对使用`new`和`delete`，否则会发生内存泄漏（memory leak），即被分配的内存再也无法使用，内存泄漏严重的，程序将由于不断寻找更多内存而终止。

使用new和 delete时,应遵守以下规则：

1. 不要使用`delete`来释放不是`new`分配的内存。

2. 不要使用`delete`释放同一个内存块两次（两个指针指向同一块内存空间）。

3. 如果使用`new`日为数组分配内存,则应使用 `delete[]`来释放。
...
**数组指针和指针数组**
* 数组指针中`*pn`与`pn[0]`等价
* C++允许指针和整数相加。加1的结果为原来的地址值加上指向的对象占用的总字节数。
* 亦可实现两指针的相减，获得两个指针的间隔。
...
ps->price;//指向结构体中的price成员，等价于(*ps).price;
```

*区分何时使用`.`,`->`来访问结构体成员：如果结构标识符是结构名，则使用`.`；如果标识符是指向结构体的指针，则使用`->`*

### 结构体
使用成员运算符`.`来访问各个成员。

### 共用体
区别于结构体的可以同时存储int,long,double类型，共用体只能存储int,long,double一种
共用体常用于节省内存
